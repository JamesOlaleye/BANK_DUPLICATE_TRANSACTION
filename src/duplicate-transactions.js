function findDuplicateTransactions(inputTrans) {
  ...
  if (typeof inputTrans === "object") {
    ...
    transaction0.sort(
      (transA, transB) => getTimeStamp(transA.time) - getTimeStamp(transB.time)
    );
    ...
    while (transaction0.length > 0) {
      ...
      while (indexOfDuplicates !== -1) {
        ...
        indexOfDuplicates = transaction0.findIndex((transaction) =>
          isDuplicate(duplicates.at(-1), transaction)
        );
      }
    ...
    function isDuplicate(transA, transB) {
      if (transA.sourceAccount !== transB.sourceAccount) return false;
      if (transA.targetAccount !== transB.targetAccount) return false;
      if (transA.amount !== transB.amount) return false;
      if (transA.category !== transB.category) return false;
...
findDuplicateTransactions(accountHolder);
